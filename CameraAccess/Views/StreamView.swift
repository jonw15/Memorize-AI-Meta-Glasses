...
struct StreamView: View {
  ...
  var body: some View {
    ...
    .onAppear {
      // Only start video stream when device is connected
      guard viewModel.hasActiveDevice else {
        print("⚠️ StreamView: RayBan Meta glasses not connected, skipping startup")
        return
      }

      // Auto-start video stream
      Task {
        print("🎥 StreamView: Starting video stream")
  ...
  private var deviceNotConnectedView: some View {
    VStack(spacing: AppSpacing.xl) {
      ...
      VStack(spacing: AppSpacing.lg) {
        ...
        Text("RayBan Meta Glasses Not Connected")
          .font(AppTypography.title2)
          .foregroundColor(.white)

        Text("Please connect your smart glasses on the home screen first,\nthen use the live stream feature")

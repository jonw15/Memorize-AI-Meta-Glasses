...
public class YouTube {
    ...
    var js: String {
        get async throws {
            ...
            // 1. Static memory cache (same process, different YouTube instance)
            if YouTube.__jsURL == jsURL, let memCached = YouTube.__js {
                print("🎬 [YouTubeKit] Player JS loaded from memory cache")
                _js = memCached
                return _js!
            }

            // 2. Persistent disk cache (survives app restart)
            if let diskCached = JSCache.load(forURL: jsURL) {
                print("🎬 [YouTubeKit] Player JS loaded from disk cache")
                _js = diskCached
                YouTube.__js = diskCached
                YouTube.__jsURL = jsURL
                return _js!
            }

            // 3. Download from network
            print("🎬 [YouTubeKit] Player JS downloading from network: \(jsURL.lastPathComponent)")
            let (data, _) = try await URLSession.shared.data(from: jsURL)
            _js = String(data: data, encoding: .utf8) ?? ""
            YouTube.__js = _js
            YouTube.__jsURL = jsURL

            // 4. Persist to disk for next app launch
            if let js = _js {
                JSCache.save(js, forURL: jsURL)
                print("🎬 [YouTubeKit] Player JS saved to disk cache (\(js.count) bytes)")
            }
